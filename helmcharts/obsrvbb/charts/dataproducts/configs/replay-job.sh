...
if [ $? == 0 ]
	...
	delete {{ .Values.gcp_bucket_name }} "derived/backup-$backup_key" >> "$DP_LOGS/$end_date-$1-replay.log"
else
	echo "$1 replay failed" >> "$DP_LOGS/$end_date-$1-replay.log"
 	rollback {{ .Values.gcp_bucket_name }} "derived/$backup_key" "backup-$backup_key" >> "$DP_LOGS/$end_date-$1-replay.log"
 	delete {{ .Values.gcp_bucket_name }} "derived/backup-$backup_key" >> "$DP_LOGS/$end_date-$1-replay.log"
fi